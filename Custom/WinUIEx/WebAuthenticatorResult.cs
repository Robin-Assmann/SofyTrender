        ...
        public WebAuthenticatorResult(Uri callbackUrl)
        {
            ...
            foreach (string key in query.Keys)
            {
                ...
                if (key == "state")
                {
                    try
                    {
                        var jsonObject = JsonNode.Parse(query[key] ?? "{}") as JsonObject;

                        if (jsonObject is not null && jsonObject.ContainsKey("state") && jsonObject["state"] is JsonValue jvalue && jvalue.TryGetValue(out string? value))
                        {
