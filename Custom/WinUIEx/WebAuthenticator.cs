...
namespace SofyTrender.Scripts.WinUIEx
{
    /// <summary>
    ...
    public sealed class WebAuthenticator
    {
        ...
        public static bool IsUriProtocolDeclared(string scheme)
        {
            if (Windows.ApplicationModel.Package.Current is null)
                return false;
            var docPath = Path.Combine(Windows.ApplicationModel.Package.Current.InstalledLocation.Path, "AppxManifest.xml");
            ...
            if (vals != null && vals["state"] is string state)
            {
                try
                {
                    ...
                    if (jsonObject is not null)
                    {
                        ...
                        if (jsonObject.ContainsKey("appInstanceId") && jsonObject["appInstanceId"] is JsonValue jvalue && jvalue.TryGetValue(out string? value))
                            vals2.Add("appInstanceId", value);
                        if (jsonObject.ContainsKey("signinId") && jsonObject["signinId"] is JsonValue jvalue2 && jvalue2.TryGetValue(out string? value2))
        ...
        private async Task<WebAuthenticatorResult> Authenticate(Uri authorizeUri, Uri callbackUri, CancellationToken cancellationToken)
        {
            ...
            {
                { "appInstanceId", AppInstance.GetCurrent().Key },
                { "signinId", taskId }
            };
